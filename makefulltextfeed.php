...
if ($options->caching) {
	$frontendOptions = array(
	   'lifetime' => ($valid_key || !$options->restrict) ? 10*60: 10*60, // cache lifetime of 10 min-dat
...
if ($options->caching) {
	...
	$output = preg_replace("/width=&quot;(\d+)(&quot;|px)/", "width=&quot;290$2", $output);
	//$output= preg_replace("/<img[^>]+\>/i", "", $output); 
	$output =preg_replace('/&lt;h1 class=(.*?)\/h1&gt;/', '', $output);
	$output =str_replace(array('h2', '</h2>'), array('p', '</p>'), $output);
	
	
	$comment='&lt;a class=&quot;SForm&quot; onclick=&quot;ShowFormComment()&quot;&gt;&lt;img src=&quot;http://vnexpress.net/Images/Y-kien-cua-ban.gif&quot; border=&quot;0&quot; style=&quot;cursor:pointer&quot; alt=&quot;Ý kiến của bạn&quot; /&gt;';
	...
	//$output =preg_replace('/&lt;h2 class=(.*?)\/h2&gt;/', '', $output);
	
	$local_fp = fopen("vne.xml", "w");
	fwrite($local_fp, $output);
	fclose($local_fp);
	ob_end_clean();
